...
import { Home, Menu, X, LogOut, Instagram } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-md transition-all">
        <div className="container mx-auto px-6 flex justify-between items-center h-24">
          ...
          <div className="hidden md:flex gap-12 text-xl font-medium items-center">
            ...
            {/* Instagram Inline Link (Desktop) */}
            <a
              href={instaUrl}
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Follow Ek Paul Foundation on Instagram (opens in new tab)"
              className="flex items-center gap-2 text-pink-600 hover:text-pink-700 font-semibold transition-all hover:scale-105"
            >
              <Instagram className="w-6 h-6" />
              <span className="hidden lg:inline">Follow on Instagram</span>
            </a>
            ...
            >
              <div className="flex flex-col items-center py-5 space-y-5 text-lg font-medium">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 text-pink-600 hover:text-pink-700 font-semibold"
                  onClick={() => setIsOpen(false)}
                  aria-label="Follow Ek Paul Foundation on Instagram (opens in new tab)"
                >
                  <Instagram className="w-6 h-6" />
      ...
      {/* Floating Sticky Instagram Button */}
      <div
        className="fixed right-4 bottom-6 z-50 flex items-center"
        aria-hidden={false}
      >
        <a
          href={instaUrl}
        ...
        >
          {/* Tooltip */}
          <span className="hidden sm:inline-block absolute -left-56 bottom-1/2 transform translate-y-1/2 whitespace-nowrap bg-black/90 text-white text-sm px-3 py-2 rounded-md shadow-md opacity-0 group-hover:opacity-100 transition-opacity">
            Follow on Instagram for regular updates
          </span>

          {/* Button */}
          <span className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-white border border-gray-200 shadow-md hover:shadow-lg transform transition-all hover:-translate-y-1 group-hover:scale-105">
